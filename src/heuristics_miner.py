...
# =============================================================================
# ADD TO END OF src/process_miner.py
# =============================================================================

import pm4py


def discover_heuristics_net(df, dependency_threshold=0.5, frequency_threshold=2):
    """
    Discover Petri net using Heuristic Miner.
    
    Wrapper function that runs heuristic mining pipeline and converts
    the result to a Petri net via pm4py.
    
    Args:
        df: pandas DataFrame with case_id, activity, timestamp columns
        dependency_threshold: Minimum dependency measure (default: 0.5)
        frequency_threshold: Minimum frequency count (default: 2)
    ...
    for trace in traces:
        for act in trace:
            activities_occurrences[act] = activities_occurrences.get(act, 0) + 1
    ...
    for a in activities:
        ...
        for b in activities:
            dep = compute_dependency_measure(a, b, normal_follows, self_loops)
            if dep > 0:
                dependency_values[a][b] = dep
    
    dep_matrix = {}
    for source in activities:
        dep_matrix[source] = {}
        for target in activities:
            if source in dependency_values and target in dependency_values[source]:
                if dependency_values[source][target] >= dependency_threshold:
                    dep_matrix[source][target] = dependency_values[source][target]
    
    edge_list = []
    for source, targets in dep_matrix.items():
        for target, value in targets.items():
            edge_list.append((source, target, value))
    
    split_result = detect_split_types(edge_list, traces, and_threshold=0.7, xor_threshold=0.3)
    splits = split_result.get('splits', {})
    
    node_objects = {}
    for act_name in activities:
        node_objects[act_name] = Node(act_name)
    
    for source, targets in dep_matrix.items():
        for target, value in targets.items():
            source_node = node_objects[source]
            target_node = node_objects[target]
            source_node.output_connections[target_node] = value
            target_node.input_connections[source_node] = value
    
    for activity, split_list in splits.items():
        if activity not in node_objects:
            continue
        source_node = node_objects[activity]
        ...
        for s in split_list:
            if s['type'] == 'AND':
                ...
                for succ_name in succs:
                    if succ_name in node_objects:
                        succ_nodes.add(node_objects[succ_name])
                
                for succ_node in succ_nodes:
                    if succ_node not in source_node.and_measures_out:
                        source_node.and_measures_out[succ_node] = set()
                    for sibling in succ_nodes:
                        if sibling != succ_node:
                            source_node.and_measures_out[succ_node].add(sibling)
    
    for activity, node in node_objects.items():
        node.and_measures_in = {}
        for target_node, sibling_set in node.and_measures_out.items():
            if target_node not in target_node.and_measures_in:
                target_node.and_measures_in = {}
            target_node.and_measures_in[node] = sibling_set.copy()
    
    heuristics_net = HeuristicsNet.__new__(HeuristicsNet)
    
    heuristics_net.nodes = node_objects
    heuristics_net.activities = activities
    heuristics_net.start_activities = starts
    heuristics_net.end_activities = ends
    heuristics_net.activities_occurrences = activities_occurrences
    heuristics_net.frequency_dfg = dict(normal_follows)
    heuristics_net.dependency_matrix = dep_matrix
    heuristics_net.dependency_values = dependency_values
    heuristics_net.edges = [(s, t, v) for s, ts in dep_matrix.items() for t, v in ts.items()]
    
    net, im, fm = pm4py.convert_to_petri_net(heuristics_net)
